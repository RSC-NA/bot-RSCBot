...
class TeamManager(commands.Cog):
    ...
    async def removeFranchise(self, ctx, *, franchise_identifier: str):
        ...
        if not franchise_data:
            await ctx.send(
                "No franchise could be found with the identifier: **{0}**".format(
                    franchise_identifier
                )
            )
            return False
    ...
    async def transferFranchise(
        ...
        if self.is_gm(new_gm):
            await ctx.send(
                '{0} already has the "General Manager" role.'.format(new_gm.name)
            )
            return False
        ...
        prompt = (
            "Transfer ownership of **{franchise}** from {old_gm} to {new_gm}?".format(
        ...
        franchise_tier_roles = await self._find_franchise_tier_roles(
            ctx, franchise_role
        )
        gm_role = self._find_role_by_name(ctx, TeamManager.GM_ROLE)
        ...
        await ctx.send(
            "{0} is the new General Manager for {1}.".format(
                new_gm.name, franchise_name
            )
        )
    ...
    async def rebrandFranchise(
        ...
        if failed:
            await ctx.send(
                ":x: Something went wrong... Only {}/{} teams have been rebranded.".format(
                    len(added), len(added) + len(failed)
                )
            )
    ...
    async def teams(self, ctx, *, franchise_tier_identifier: str):
        ...
        await ctx.send(
            "No tier, franchise, prefix, or GM with name: {0}".format(
                franchise_tier_identifier
            )
        )
    ...
    async def roster(self, ctx, *, team_name: str):
        ...
        if found:
            ...
            if franchise_role is None or tier_role is None:
                await ctx.send(
                    "No franchise and tier roles set up for {0}".format(team)
                )
                return
            await ctx.send(embed=await self.create_roster_embed(ctx, team))
        else:
    ...
    async def listTiers(self, ctx):
        ...
        if tiers:
            await ctx.send(
                "Tiers set up in this server: {0}".format(
    ...
    async def captains(self, ctx, *, franchise_tier_prefix: str):
        ...
        await ctx.send(
            "No franchise, tier, or prefix with name: {0}".format(franchise_tier_prefix)
        )
    ...
    async def teamRoles(self, ctx, team_name: str):
        ...
        if franchise_role and tier_role:
            await ctx.send(
                "Franchise role for {0} = {1}\nTier role for {0} = {2}".format(
                    team_name, franchise_role.name, tier_role.name
                )
            )
        else:
            await ctx.send(
                "No franchise and tier roles set up for {0}".format(team_name)
            )
    ...
    async def freeAgents(self, ctx, tier: str, filter=None):
        ...
        for role in ctx.guild.roles:
            if role.name.lower() == tier_name.lower():
                color = role.color
        embed = discord.Embed(
            title="{0} Free Agents".format(tier_name),
            color=color,
            description=message,
            thumbnail=ctx.guild.icon_url,
        )
    ...
    async def format_roster_info(self, ctx, team_name: str):
        ...
        try:
            ...
            team_members = await player_ratings.sort_members_by_rating(
                ctx, team_members
            )
        except:
            pass
    ...
    async def _format_team_member_for_message(self, ctx, member, *args):
        ...
        try:
            ...
            (
                ...
                losses,
                rating,
            ) = await player_ratings.get_player_record_and_rating_by_id(ctx, member.id)
            if wins is not None:
    ...
    async def _remove_tier(self, ctx, tier_name):
        if len(await self._find_teams_for_tier(ctx, tier_name)) > 0:
            ...
            try:
                ...
                await ctx.send("{0} does not seem to be a tier.".format(tier_name))
                return
            await self._save_tiers(ctx, tiers)
            return True
    ...
    async def _set_user_nickname_prefix(self, ctx, prefix: str, user: discord.member):
        try:
            if prefix:
                await user.edit(
                    nick="{0} | {1}".format(prefix, self.get_player_nickname(user))
                )
            else:
                await user.edit(nick=self.get_player_nickname(user))
        except discord.Forbidden:
            await ctx.send(
                "Changing nickname forbidden for user: {0}".format(user.name)
            )
    ...
    async def _get_franchise_emoji(self, ctx, franchise_role):
        ...
        if prefix:
            ...
            for emoji in emojis:
                if (
                    emoji.name.lower() == prefix.lower()
                    or emoji.name.lower() == gm_name.lower()
                ):
                    return emoji
