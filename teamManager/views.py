...
class AddFranchiseView(discord.ui.View):
    ...
    def __init__(self, cog: 'TeamManager', ctx: Context, name: str, prefix: str, gm: Union[discord.Member, discord.User], timeout: float = 10.0):
        super().__init__()
        self.cog = cog
        self.ctx = ctx
    ...
    async def confirm(self, interaction:discord.Interaction, button: discord.ui.Button):
        ...
        if franchise_role:
            ...
            await self.msg.edit(embed=ErrorEmbed(description="Error creating franchise role."), view=None)
        self.stop()
...
class RemoveFranchiseView(discord.ui.View):
    ...
    def __init__(self, cog: 'TeamManager', ctx: Context, role: discord.Role, name: str, prefix: str, gm: Union[discord.Member, discord.Role], timeout: float = 10.0):
        super().__init__()
        self.cog = cog
        self.ctx = ctx
        self.author = ctx.author
    ...
    async def confirm(self, interaction:discord.Interaction, button: discord.ui.Button):
        ...
        if len(franchise_teams) > 0:
            await self.msg.edit(embed=ErrorEmbed(description="Cannot remove a franchise that has teams enrolled"), view=None)
        else:
            gm_role = self.cog._find_role_by_name(self.ctx, self.cog.GM_ROLE)
...
class TransferFranchiseView(discord.ui.View):
    ...
    def __init__(self, cog: 'TeamManager', ctx: Context, role: discord.Role, name: str, prefix: str, old_gm: Union[discord.Member, discord.Role], new_gm: Union[discord.Member, discord.User], timeout: float = 10.0):
        super().__init__()
        self.cog = cog
        self.ctx = ctx
        self.author = ctx.author
    ...
    async def prompt(self):
        ...
        add_embed = discord.Embed(
            ...
            description=
                f"Transfer ownership of **{self.name}** from **{self.old_gm.display_name}** to **{self.new_gm.display_name}**\n\n"
                "Are you sure?",
            color=discord.Color.blue()
        )
        self.msg = await self.ctx.send(embed=add_embed, view=self)
    ...
    async def confirm(self, interaction:discord.Interaction, button: discord.ui.Button):
        ...
        franchise_tier_roles = await self.cog._find_franchise_tier_roles(self.ctx, self.role)
        gm_role = self.cog._find_role_by_name(self.ctx, self.cog.GM_ROLE)
...
class RebrandFranchiseView(discord.ui.View):
    ...
    def __init__(self, cog: 'TeamManager', ctx: Context, role: discord.Role, old_name: str, prefix: str, gm: Union[discord.Member, discord.User], new_name: str, new_teams: Sequence[str], old_teams: Sequence[str], timeout: float = 30.0):
        super().__init__()
        self.cog = cog
        self.ctx = ctx
        self.author = ctx.author
    ...
    async def prompt(self):
        ...
        self.tier_roles = [role[1] for role in [await self.cog._roles_for_team(self.ctx, team) for team in self.old_teams]]
        log.debug
        ...
        add_embed.add_field(name="Tier", value="\n".join(tier.mention for tier in self.tier_roles), inline=True)
        add_embed.add_field(name="Team Name", value="\n".join(team for team in self.new_teams), inline=True)
        self.msg = await self.ctx.send(embed=add_embed, view=self)
    ...
    async def confirm(self, interaction:discord.Interaction, button: discord.ui.Button):
        ...
        # Fix player prefixes
        await self.cog._set_user_nickname_prefix(self.ctx, prefix=self.prefix, user=self.gm)
        ...
        for tier in self.tier_roles:
            for player in await self.cog.members_from_team(self.role, tier):
                ...
                await self.cog._set_user_nickname_prefix(self.ctx, prefix=self.prefix, user=player)
        ...
        for i in range(len(self.tier_roles)):
            ...
            if await self.cog._add_team(self.ctx, new_team, self.gm.name, tier_role.name):
                added.append(new_team)
            else:
                failed.append(new_team)

        if failed:
            await self.msg.edit(embed=ErrorEmbed(description=f"Something went wrong... Only {len(added)}/{len(added) + len(failed)} teams have been rebranded."))
        else:
            success_embed = discord.Embed(
                title="Success",
