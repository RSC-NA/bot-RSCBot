...
class StatsManager(commands.Cog):
    ...
    async def teamStats(self, ctx, *, team_name: str):
        ...
        if found:
            ...
            embed = await self.get_team_stats_embed(
                ctx, team, franchise_role, tier_role, stats
            )
    ...
    async def playerStats(self, ctx, *, player: discord.Member = None):
        ...
        if sr.LEAGUE_ROLE_NAME not in [role.name for role in player.roles]:
            return await ctx.send(
                ":x: This command is only supported for active players."
            )
    ...
    async def get_player_stats(
        self, guild: discord.Guild, player: discord.Member, team: str
    ):
    ...
    async def get_player_stats_embed(self, ctx, player, team, player_stats):
        ...
        embed = discord.Embed(
            title=f"{player_name}'s Player Stats Card", color=tier_role.color
        )
        ...
        if under_contract:
            ...
            contract = (
                ...
                if sr.PERM_FA_ROLE_NAME in [role.name for role in player.roles]
                else "Free Agent"
            )
            team_info += f"\n**Contract:** {contract}"
        ...
        if under_contract:
            if player_stats:
                for stat in sr.INCLUDE_PLAYER_STATS:
                    ...
                    embed.add_field(
                        name=stat_title, value=player_stats.get(stat, "N/A")
                    )  # , inline=False)
            else:
    ...
    async def get_team_stats_embed(
        self,
        ctx,
        team,
        ...
        for stat in sr.INCLUDE_TEAM_STATS:
            stat_title = self.get_code_title(stat)
            embed.add_field(
                name=stat_title, value=team_stats.get(stat, "N/A")
            )  # , inline=False)
