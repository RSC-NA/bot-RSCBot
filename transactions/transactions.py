...
# region commands
    ...
    async def trade(self, ctx, user: discord.Member, new_team_name: str, user_2: discord.Member, new_team_name_2: str):
        ...
        gm_name_1 = await self._get_gm_name(ctx, franchise_role_1)
        gm_name_2 = await self._get_gm_name(ctx, franchise_role_2)
    ...
    async def sub(self, ctx, user: discord.Member, team_name: str, subbed_out_user: discord.Member = None) -> NoReturn:
        ...
        if franchise_role in user.roles and team_tier_role in user.roles:
            if list(set([free_agent_role, perm_fa_role]) & set(user.roles)):
                ...
                await user.remove_roles(team_tier_role)
            gm = await self._get_gm_name(ctx, franchise_role)
