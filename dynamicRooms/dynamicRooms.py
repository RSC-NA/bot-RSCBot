...
class DynamicRooms(commands.Cog):
    ...
    async def getDynamicCategories(self, ctx):
        ...
        message = (
            "The following categories have been set as dynamic:\n - "
    ...
    async def _member_joins_voice(
        ...
        if dynamic_vc:
            if len(voice_channel.members) == 1:
                ...
                if (
                    ...
                    and voice_channel.category.id
                    in await self._get_dynamic_categories(member.guild)
                ):
        ...
        ) == voice_channel.user_limit and not await self._is_hiding(voice_channel):
    ...
    async def _move_to_last(self, voice_channel: discord.VoiceChannel):
        ...
        if last_index > voice_channel.position:
            await voice_channel.edit(
                position=last_index + 3
            )  # 3 is an arbitrary number to cover a race condition - sometimes moves to 2nd last
